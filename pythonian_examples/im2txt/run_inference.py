...
def preplabelImg(checkpoint_path,vocab_file):
  graph = tf.Graph()
  ...
  generator = caption_generator.CaptionGenerator(model, vocab)
  return model, vocab,graph,restore_fn,generator


def labelImg(checkpoint_path,vocab_file,input_files):
  g = tf.Graph()
  with g.as_default():
    model = inference_wrapper.InferenceWrapper()
    restore_fn = model.build_graph_from_config(configuration.ModelConfig(),
  ...
  with tf.Session(graph=g) as sess:
    ...
    for filename in filenames:
      ...
      for i, caption in enumerate(captions):
        ...
        sentence = " ".join(sentence)
        print("  %d) %s (p=%f)" % (i, sentence, math.exp(caption.logprob)))
        caption_result[filename].append(sentence)
    return caption_result


def labelImgg(model,vocab,g,restore_fn,generator,sess,input_files):
  filenames = []
  for file_pattern in input_files.split(","):
    filenames.extend(tf.gfile.Glob(file_pattern))
  tf.logging.info("Running caption generation on %d files matching %s",
